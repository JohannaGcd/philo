...
static int	mutex_error_handler(t_mutex_operation operator, int status)
{
	if (status == 0)
		...
		return(print_msg("Invalid mutex: the value specified by mutex is invalid.\n", NULL, EXIT_FAILURE));
	else if (EINVAL == status && operator == MUTEX_CREATE)
		return(print_msg("Invalid attributes: the value specified by attr is invalid.\n", NULL, EXIT_FAILURE));
...
int mutex_operator(pthread_mutex_t *mutex, t_mutex_operation operation)
{
	...
	if (operation == MUTEX_LOCK)
		...
		return (print_msg("Wrong operation code for mutex.\n", NULL, EXIT_FAILURE));
